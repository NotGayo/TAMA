...
import Model.MiniCartas.FactoryCartas.CartaFactory;
...
public class Tablero extends Observable {
	...
	private Tablero() {
		...
		for(int i=0;i<this.parejas;i++) {
			Carta k = CartaFactory.getCF().crearCarta(i);
			c.add(k);
			c.add(k);
	...
	public void seleccionarCarta(int fila,int col, int pOrder) {
		...
		if (c.estado.getEstado().equals("SinVoltear")) {
	...
	public void acabarPartida() {
		setChanged();
		if(this.parejasEncontradas==this.parejas || seconds == 20) {
